...
class SCDownloader:
    ...
    def load_config(self):
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                print(f"加载配置文件失败: {e}")
        return {"mirrors": [DEFAULT_MIRROR], "current_mirror": DEFAULT_MIRROR}
    ...
    def add_mirror(self, mirror_url):
        ...
        if mirror_url not in self.mirrors:
            ...
            self.save_config()
            print(f"镜像 {mirror_url} 已添加并设为当前镜像")
        else:
            print(f"镜像 {mirror_url} 已存在")
    ...
    def download_file(self, url, output_path, show_progress=True):
        ...
        try:
            ...
            print("警告: 使用后备下载方法。安装 requests 和 tqdm 以获得更好的进度条。")
        ...
        if plugin_name == "scd":
            ...
            if self.compare_versions(local_version, remote_version) < 0:
                ...
                if self.download_file(scd_url, scd_path):
                    ...
                    print("scd 更新成功！")
                    print(f"请运行新的 scd{'文件' if system == 'Windows' else '可执行文件'}")
                    return True
                else:
                    print("下载更新失败")
                    return False
            else:
                print("scd 已是最新版本，无需更新")
                return True
        else:
            # 更新普通插件
            print(f"正在更新插件: {plugin_name}")
    ...
    def list_plugins(self, mode="local"):
        ...
        if mode == "local":
            ...
            print("正在列出网络插件...")
            print(f"从以下地址获取插件: {self.current_mirror}")
            try:
                # 尝试从 API 获取插件列表
                import requests
                ...
                print("错误: 请安装必要的包: pip install requests")
            except Exception as e:
                print(f"获取插件列表失败: {e}")
                print("请检查网络连接或镜像 URL")
    ...
    def run(self, args):
        ...
        if args.command == "install":
            ...
            print("SCL 插件下载器 (scd)")
            print(f"版本: 1.0.0")
            print(f"当前镜像: {self.current_mirror}")
            print(f"可用镜像: {len(self.mirrors)}")
            print(f"CPU 核心数: {self.cpu_cores}")
            print(f"下载线程数: {self.threads}")
